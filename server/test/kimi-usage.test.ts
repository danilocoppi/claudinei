...
describe('limites de plano do Kimi', () => {
  it('normaliza a cota principal como semanal e a janela de 300min como sessão', async () => {
    const svc = createKimiUsageService({ credentialsPath: credsPath, fetchFn: okFetch(), now: () => NOW })
  ...
  it('token expirado não gera requisição (a CLI é quem renova)', async () => {
    ...
    const svc = createKimiUsageService({ credentialsPath: credsPath, fetchFn, now: () => NOW })
  ...
  it('sem credencial, HTTP ruim ou corpo inesperado → [] (o card só perde as barras)', async () => {
    const semArquivo = createKimiUsageService({ credentialsPath: '/nao/existe.json', fetchFn: okFetch(), now: () => NOW })
    await expect(semArquivo.getLimits()).resolves.toEqual([])

    const http401 = createKimiUsageService({ credentialsPath: credsPath, fetchFn: okFetch({}, 401), now: () => NOW })
    await expect(http401.getLimits()).resolves.toEqual([])

    const lixo = createKimiUsageService({ credentialsPath: credsPath, fetchFn: okFetch({ usage: { limit: '0', used: 'x' } }), now: () => NOW })
  ...
  it('respeita KIMI_CODE_BASE_URL (self-hosted/proxy) no endpoint', async () => {
    ...
    await createKimiUsageService({ credentialsPath: credsPath, fetchFn, now: () => NOW }).getLimits()
