...
- **Codex context control**: context usage from the native App Server, **Compact now**, a compaction timer, and the same global percentage threshold used by Claude. Existing conversations and terminal handoff are preserved. [Details](docs/codex-context.md).
- **Context meter**: each engine tab shows the conversation's latest context usage against its active model window. Claude reports usage at the end of a turn; Codex supplies usage and the active window through its native protocol.
- **Per-session ⚙**: hot-swap **model**, **effort** (persisted) and supported **permission modes**; **Compact now** frees window space on demand, and admins can set a global **auto-compact** threshold for Claude and Codex sessions.
- **Messages during Codex work**: additions join the active turn in order, or start the next turn if the current one has ended. Delivery failures appear in the chat.
- **Frontend performance**: buffered text updates, independent streaming rendering, and spaced activity pulses reduce chat and graphics work. [Measurements and reproduction](docs/frontend-performance.md).
