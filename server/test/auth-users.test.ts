...
describe('users: CRUD', () => {
  it('create/list/get sem expor hash; count reflete', () => {
    ...
    const u = svc.create({ username: 'master', password: 'abcd1234', isAdmin: true })
  ...
  it('getByUsername devolve hash verificável e tokenVersion', () => {
    const svc = createUsersService(db)
    const u = svc.create({ username: 'ana', password: 'segredo1' })
    const row = svc.getByUsername('ana')!
    expect(verifyPassword('segredo1', row.passwordHash)).toBe(true)
  ...
  it('username duplicado e senha curta rejeitam', () => {
    const svc = createUsersService(db)
    svc.create({ username: 'ana', password: 'abcd1234' })
    expect(() => svc.create({ username: 'ana', password: 'abcd1234' })).toThrow('username_taken')
    expect(() => svc.create({ username: 'bia', password: 'abc' })).toThrow('password_too_short')
    expect(() => svc.create({ username: '  ', password: 'abcd1234' })).toThrow('username_required')
  })

  it('update troca projetos, admin e senha (senha bumpa tokenVersion)', () => {
    const svc = createUsersService(db)
    svc.create({ username: 'root', password: 'abcd1234', isAdmin: true })
    const u = svc.create({ username: 'ana', password: 'abcd1234', projectIds: [p1.id, p2.id] })
    ...
    svc.update(u.id, { password: 'novaSenha1' })
    expect(verifyPassword('novaSenha1', svc.getByUsername('ana')!.passwordHash)).toBe(true)
  ...
  it('remove apaga user e vínculos', () => {
    const svc = createUsersService(db)
    svc.create({ username: 'root', password: 'abcd1234', isAdmin: true })
    const u = svc.create({ username: 'ana', password: 'abcd1234', projectIds: [p1.id] })
    svc.remove(u.id)
  ...
  it('projectId inexistente na criação viola FK e lança', () => {
    const svc = createUsersService(db)
    svc.create({ username: 'root', password: 'abcd1234', isAdmin: true })
    expect(() => svc.create({ username: 'x', password: 'abcd1234', projectIds: [9999] })).toThrow()
  })
  ...
  it('último admin não pode ser removido nem des-adminado', () => {
    const svc = createUsersService(db)
    const root = svc.create({ username: 'root', password: 'abcd1234', isAdmin: true })
    expect(() => svc.remove(root.id)).toThrow('last_admin')
    expect(() => svc.update(root.id, { isAdmin: false })).toThrow('last_admin')
    svc.create({ username: 'root2', password: 'abcd1234', isAdmin: true })
...
describe('users: lockout (clock injetado)', () => {
  it('5ª falha tranca por 15 min; sucesso pós-expiração destrava', () => {
    ...
    const u = svc.create({ username: 'ana', password: 'abcd1234' })
...
describe('users: revogação', () => {
  it('bumpTokenVersion incrementa um; revokeAll incrementa todos', () => {
    const svc = createUsersService(db)
    const a = svc.create({ username: 'a', password: 'abcd1234', isAdmin: true })
    const b = svc.create({ username: 'b', password: 'abcd1234' })
