...
describe('setup do master', () => {
  ...
  it('setup cria o admin, seta cookie httpOnly/strict e só funciona uma vez', async () => {
    const res = await app.inject({ method: 'POST', url: '/api/auth/setup', payload: { username: 'root', password: 'abcd1234' } })
    ...
    const again = await app.inject({ method: 'POST', url: '/api/auth/setup', payload: { username: 'x', password: 'abcd1234' } })
    expect(again.statusCode).toBe(403)
  })

  it('setup de fora do loopback é recusado (o hook já barra com 403)', async () => {
    const res = await app.inject({ method: 'POST', url: '/api/auth/setup', payload: { username: 'x', password: 'abcd1234' }, remoteAddress: '10.0.0.9' })
...
describe('login/logout/lockout', () => {
  ...
  it('credencial errada → 401 genérico; certa → cookie', async () => {
    ...
    const ok = await login('root', 'abcd1234')
  ...
  it('5 falhas → 429 com retryAfterMs', async () => {
    ...
    const res = await login('root', 'abcd1234') // senha certa, mas trancado
...
describe('troca de senha', () => {
  ...
  it('senha atual errada → 400; certa troca, re-emite cookie e mata o token antigo', async () => {
    const c1 = cookieOf(await login('root', 'abcd1234'))
    const bad = await app.inject({ method: 'POST', url: '/api/auth/password', cookies: c1, payload: { currentPassword: 'x', newPassword: 'novaSenha1' } })
    expect(bad.statusCode).toBe(400)
    const ok = await app.inject({ method: 'POST', url: '/api/auth/password', cookies: c1, payload: { currentPassword: 'abcd1234', newPassword: 'novaSenha1' } })
    expect(ok.statusCode).toBe(200)
    ...
    expect((await login('root', 'novaSenha1')).statusCode).toBe(200)
  })
})
...
describe('admin: users CRUD + revoke-all', () => {
  ...
  beforeEach(async () => {
    ...
    adminCookie = cookieOf(await login('root', 'abcd1234'))
  })
  ...
  it('CRUD completo com cookie de admin', async () => {
    ...
    ins.run(2, 'p2', '/tmp/p2')
    ins.run(3, 'p3', '/tmp/p3')
    const created = await app.inject({ method: 'POST', url: '/api/auth/users', cookies: adminCookie, payload: { username: 'ana', password: 'abcd1234', projectIds: [1] } })
  ...
  it('não-admin leva 403 no CRUD e no revoke-all', async () => {
    await app.inject({ method: 'POST', url: '/api/auth/users', cookies: adminCookie, payload: { username: 'ana', password: 'abcd1234' } })
    const anaCookie = cookieOf(await login('ana', 'abcd1234'))
  ...
  it('editar um usuário derruba os sockets dele (onUserInvalidated)', async () => {
    const created = await app.inject({ method: 'POST', url: '/api/auth/users', cookies: adminCookie, payload: { username: 'ana', password: 'abcd1234' } })
